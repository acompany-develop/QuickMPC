...
from quickmpc.proto import libc_to_manage_v0_pb2, libc_to_manage_v0_pb2_grpc
...
class LibToManageServiceServicer \
    ...
    def ExecuteComputation(self, request, context):
        if len(request.arg.src) > 0 \
            ...
            return libc_to_manage_v0_pb2.ExecuteComputationResponse()
        ...
        res = libc_to_manage_v0_pb2.ExecuteComputationResponse(
            job_uuid="jobjobjob"
        )
        return res

    def GetComputationResult(self, request, context):
        yield libc_to_manage_v0_pb2.GetComputationResultResponse(
            result=["1"],
    ...
    def GetDataList(self, request, context):
        res = libc_to_manage_v0_pb2.GetDataListResponse(
            result="[]"
        )
        return res

    def GetJoinTable(self, request, context):
        res = libc_to_manage_v0_pb2.GetJoinTableResponse(
            is_ok=True,
            result="['1']",
            schema=["s"]
        )
    ...
    def GetJobErrorInfo(self, request, context):
        res = libc_to_manage_v0_pb2.GetJobErrorInfoResponse(
...
def serve_v0(num: int):
    """ server setting """
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    libc_to_manage_v0_pb2_grpc.add_LibcToManageServicer_to_server(
