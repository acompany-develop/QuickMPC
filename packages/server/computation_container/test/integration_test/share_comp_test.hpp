...
TEST(ShareCompTest, SubUnboundedFanInOr)
{
    ...
    QMPC_LOG_INFO("b_rec = {}", b_rec.getVal());
...
TEST(ShareCompTest, SubBitwiseLessThan)
{
    ...
    for (int loop = 0; loop < 10; loop++)
    {
        ...
        QMPC_LOG_INFO("recons_ret = {}", recons_ret.getVal());
...
TEST(ShareCompTest, SubHalfLessThan)
{
    ...
    for (int loop = 0; loop < 10; loop++)
    {
        ...
        // デバッグ用出力
        QMPC_LOG_INFO("a_rec = \t{}", a_rec.getVal());
        QMPC_LOG_INFO("p = \t\t{}", (PrimeField::p / 2) + 1);
        QMPC_LOG_INFO("recons_ret = \t{}", recons_ret.getVal());
...
TEST(ShareCompTest, IntervalTest)
{
    ...
    QMPC_LOG_INFO("[TestInterval 1] ret = {}", ret.getVal());
    ...
    QMPC_LOG_INFO("[TestInterval 2] ret = {}", ret.getVal());
...
TEST(ShareCompTest, ComparisonTest)
{
    ...
    QMPC_LOG_INFO("[TestComparison 1] ret = {}", ret.getVal());
    EXPECT_EQ(ret.getVal(), 1);
    // Test2 (a < bが成立しない場合)
    ...
    QMPC_LOG_INFO("[TestComparison 2] ret = {}", ret.getVal());
    EXPECT_EQ(ret.getVal(), 0);
}
...
TEST(ShareCompTest, EqualityTest)
{
    ...
    QMPC_LOG_INFO("[TestEquality 1] ret = {}", ret.getVal());
    EXPECT_EQ(ret.getVal(), 1);
    // Test2 (a = bが成立しない場合)
    ...
    open(f_share);
    ret = recons(f_share);
    QMPC_LOG_INFO("[TestEquality 2] ret = {}", ret.getVal());
    EXPECT_EQ(ret.getVal(), 0);
}
...
TEST(ShareCompTest, ShareCompMulBulk)
{
    ...
    QMPC_LOG_INFO("TestShareCompMulBulk End !!");
