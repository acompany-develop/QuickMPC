...
// 2^256 - 189
// //bm::cpp_int prime =
// 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF43_cppui;
...
template <typename B, typename T, typename P> B pow(B base, T a, P prime) {
  B ret{1};
  while (a > 0) {
    if (a & 1)
      (ret *= base) %= prime;
...
template <typename T> T recv(size_t size, int port) {
  asio::io_service io_service;
...
TEST(OTTest, ot1) {
  ...
  // std::uniform_int_distribution<> rand100(0, 100000);  // [0, 99]
  // 範囲の一様乱数 std::vector<std::pair<bm::cpp_int, bm::cpp_int>> ab; int i =
  // 1; for (auto &a : k)
  // {
  //     a = rand100(mt);
  ...
  //     std::cout << "h_inv is " << powm(h, a * i * (prime -2), prime) <<
  //     std::endl; auto gk = powm(g, a, prime); auto h_ = powm(h, i, prime);
  //     //std::cout << "h inv integer is " <<
  //     boost::integer::mod_inverse(h_,prime) << std::endl; auto h_inv =
  //     powm(h_, a * i * (prime - 2), prime); auto b_ = powm(B, a, prime);
...
TEST(OTTest, ot5) {
  ...
  std::cout << "s , s_dash, r_dash" << s << " " << s_dash << " " << r_dash
            << std::endl;
  // recvier
  auto beta = ((pow(g, s, prime) % prime) * pow(h, r, prime)) % prime;

  auto beta_dash =
      ((pow(g, s_dash, prime) % prime) * pow(h, r_dash, prime)) % prime;
...
TEST(OTTest, group) { qmpc::Group g; }