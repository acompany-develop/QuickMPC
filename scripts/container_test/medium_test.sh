...
setup() {
    rm -f /tmp/QuickMPC && docker-compose $COMPOSE_FILES_OPT down -v
}

# run処理を記述する関数
# NOTE: この関数は例外的にワンライナーで書かなくて良い
run() {
    # DooDでmountしてコンテナを立てようとするとpathがデーモンのあるhostから参照されてしまう
    # コンテナ内からhostのpathが見えず指定できないため，明示的な場所にQuickMPCを作ってここを指定するようにする
    ln -s $(pwd)/../ /tmp/QuickMPC && docker-compose $COMPOSE_FILES_OPT up medium_container
}
