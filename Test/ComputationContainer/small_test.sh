...
run() {
    docker-compose -f docker-compose.yml run -T small_cc /bin/sh -c "cd /QuickMPC && tar xzfP cache.tar.gz --warning=no-timestamp && bazel test //Test/UnitTest:all --config=debug --test_env=IS_TEST=true --test_output=all"
}
